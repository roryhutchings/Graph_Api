...
const ReviewType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    location: {
      //queries main database with commonuser to get all location information, gets passed a locationid in GraphQLID format, returns all location information
      type: new GraphQLList(locationstypes.LocationType),
      resolve(parent, args) {
        return new queries.Locations(pools.mainpoolcommonquery).GetLocationByID(
          args.id
        );
      },
    },
    ...
    user: {
      //queries main database with commonuser to get all user information, gets passed a userid in graphqlID format, returns all user information
